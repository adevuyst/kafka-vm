...
if [ ! -e confluent ]; then
    ...
    if [ ! -e confluent-5.0.0-2.11.tar.gz ]; then
        wget http://packages.confluent.io/archive/5.0/confluent-5.0.0-2.11.tar.gz
    fi
    popd

    tar xvzf /tmp/vagrant-downloads/confluent-5.0.0-2.11.tar.gz
fi
